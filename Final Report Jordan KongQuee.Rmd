...
3. Are there associations between types of arrests and the demographic information about the individual arrested? If the age, race, and gender of a person are known, could the person's most likely arrest category be predicted?
...
#####Daily Arrests without Liquor Law
```{r, echo=FALSE}
...
***Are there associations between types of arrests and the demographic information about the individual arrested? If the age, race, and gender of a person are known, could the person's most likely arrest category be predicted?***
...
```{r, echo=FALSE}
`Age Range` <- c("Between 18 and 24", "Between 25 and 44", "Between 45 and 64", "65 and over")
`Population` <- c("38.1%", "28.6%", "22.3%", "11.1%")
a <- filter(arrests.1, Age < 18) %>% nrow
...
`Arrested` <- c((b/(b+c+d+e))*100, (c/(b+c+d+e))*100, (d/(b+c+d+e))*100, (e/(b+c+d+e))*100) %>% round(digits = 1) %>% as.character %>% paste("%", sep="")
tibble(`Age Range`, `Population`, `Arrested`)
```
...
a1 <- arrests.1 %>%
        ...
        ggplot(aes(x= Age)) + geom_histogram(bins = 25, fill="coral2", color = "coral4") + ggtitle("DRUG/NARCOTIC VIOLATION")

a2 <- arrests.1 %>%
        filter(Category == "LIQUOR LAW VIOLATION") %>%
        ggplot(aes(x= Age)) + geom_histogram(bins = 25, fill="coral2", color = "coral4") + ggtitle("LIQUOR LAW VIOLATION")

a3 <- arrests.1 %>%
        filter(Category == "DRIVING") %>%
        ggplot(aes(x= Age)) + geom_histogram(bins = 25, fill="coral2", color = "coral4") + ggtitle("DRIVING")

a4 <- arrests.1 %>%
        filter(Category == "ASSAULT") %>%
        ggplot(aes(x= Age)) + geom_histogram(bins = 25, fill="coral2", color = "coral4") + ggtitle("ASSAULT")

a5 <- arrests.1 %>%
        filter(Category == "PANHANDLING") %>%
        ggplot(aes(x= Age)) + geom_histogram(bins = 25, fill="coral2", color = "coral4") + ggtitle("PANHANDLING")

a6 <- arrests.1 %>%
        filter(Category == "NOISE COMPLAINT") %>%
        ggplot(aes(x= Age)) + geom_histogram(bins = 25, fill="coral2", color = "coral4") + ggtitle("NOISE COMPLAINT")
```
...
To visually analyze the probabilities computed above, we proceed by displaying six charts, one for each of six arrest categories. Each chart 
